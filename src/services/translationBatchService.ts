...
export const syncAndTranslateBatch = async (options?: {
  bggIds?: number[];       // 직접 지정 시 이 목록 사용 (startRank/endRank보다 우선)
  startRank?: number;      // CSV 랭킹 범위 시작 (기본: 1)
  endRank?: number;        // CSV 랭킹 범위 끝 (기본: 3000)
  ...
  if (options?.bggIds) {
    targetIds = options.bggIds;
  } else {
    const startRank = options?.startRank ?? 1;
    const endRank   = options?.endRank   ?? 3000;
    const allGames  = await loadTopRankedGamesFromCsv(endRank);
    targetIds = allGames
      .filter(g => g.rank >= startRank && g.rank <= endRank)
      .map(g => g.bggId);
  }
